...
import { BACKEND_PORT } from '../config.json'; // server port
...
function Home () {
  ...
  const sendGetDetailListing = (id) => {
    ...
    return fetch(`http://localhost:${BACKEND_PORT}/listings/${id}`, body);
  }
  ...
  const getAllListing = async (email) => {
    ...
    const response = await fetch(`http://localhost:${BACKEND_PORT}/listings`, body);
    ...
    if (response.status === 200) {
      ...
      if (token) {
        const allBookingsResponse = await fetch(`http://localhost:${BACKEND_PORT}/bookings`, body);
